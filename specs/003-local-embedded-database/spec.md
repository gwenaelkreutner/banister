...
concurrent access, and backup and restore.

**Explicitly not in scope**: migrating existing hosted data. The deployment starts with a fresh account
rather than carrying data across — see Assumptions.
...
- **SC-007**: Routine operations remain responsive against a store holding several years of history.
...
- **No data carry-over.** The author is starting this deployment with a fresh account rather than migrating
  existing hosted data — a deliberate decision made once the engine change was otherwise complete, not a
  gap in this feature. It supersedes an earlier draft of this spec, which required carrying locally
  originated data across (plans, profile, adherence history, conversation history) and included a
  behavioural equivalence check comparing the coach's answers before and after. That requirement, its
  acceptance scenario, and its success criterion have been removed rather than left unimplemented, because
  nothing in this feature depends on them and no future work is deferred by their absence.
- A consequence of the fresh start: authorization records for the previous sport-data provider (access and
  refresh tokens) are not carried into the new deployment either — the athlete reconnects rather than
  inheriting old credentials. This is independent of, and does not change, when the table backing those
  records is itself dropped from the schema: that remains spec 002's responsibility, since the provider it
  authorizes is still the one in active use until spec 002 replaces it.
- Only the author has real data, and this deployment now starts without any. No third-party migration path
